...
public class EmpruntDialog extends javax.swing.JDialog {
    private final JComboBox<Lecteur> lecteurCb = new JComboBox<>();
    private final JComboBox<Livre> livreCb = new JComboBox<>();
    ...
    public static void create(Component owner, Runnable onChanged) {
        new EmpruntDialog(owner, onChanged).setVisible(true);
    }

    private EmpruntDialog(Component owner, Runnable onChanged) {
    ...
    private void build() {
        ...
        top.add(Dialogs.title("Nouvel emprunt"), BorderLayout.WEST);
        ...
        for (Livre l : livres) livreCb.addItem(l);
        ComboStyle.apply(lecteurCb);
        ComboStyle.apply(livreCb);
        ...
        gbc.gridy++;
        form.add(fieldLabel("Livre"), gbc);
        gbc.gridy++;
        livreCb.setPreferredSize(new Dimension(0, 46));
        form.add(livreCb, gbc);

        gbc.gridy++;
        form.add(fieldLabel("Date d'emprunt"), gbc);
        gbc.gridy++;
    ...
    private void doSave() {
        if (lecteurCb.getSelectedItem() == null || livreCb.getSelectedItem() == null) {
            errorLabel.setText("Sélectionnez un lecteur et un livre.");
            return;
        }
        LocalDate date;
        try {
        ...
        Livre livre = (Livre) livreCb.getSelectedItem();
        int id = DatabaseConnection.nextId("emprunt", "id_emprunt");
        DatabaseConnection.insertEmprunt(new Emprunt(id, lecteur, livre, date));
        Toast.show(this, "Emprunt enregistré");
